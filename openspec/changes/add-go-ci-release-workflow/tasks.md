...
- [ ] 1.1 Add `cliff.toml`, adapted from `dtrpg-sdk/rust/cliff.toml`: same commit-group
...
- [ ] 1.2 Seed `CHANGELOG.md` with an `[Unreleased]` header matching the Rust SDK's
  changelog format.

## 2. CI Workflow (`go-ci-workflow`)

- [ ] 2.1 Add `.github/workflows/ci.yaml`: triggers on push to `develop`; checks out with
  `submodules: recursive`; runs `gofmt -l .` (fails if non-empty), `go vet ./...`,
  `golangci-lint run ./...`, `go test -race ./...`.
- [ ] 2.2 Add `.github/workflows/pr.yaml`: triggers on pull_request targeting `master` and
  `develop`; runs the same four checks as `ci.yaml`.
- [ ] 2.3 Verify both workflows pass against the current `develop` HEAD (post
  `implement-go-sdk` merge) before merging this change.

## 3. Release Preparation and Tagging (`go-module-release-publishing`)

- [ ] 3.1 Add `.github/workflows/prepare-release.yaml` (`workflow_dispatch`): install
...
- [ ] 3.2 Add `.github/workflows/tag-release.yaml`: triggers on pull_request `closed`
...
- [ ] 4.1 Add `.github/workflows/release.yaml`: triggers on `v*` tag push (and
...
- [ ] 4.2 Add the Go module proxy indexing step: `GOPROXY=proxy.golang.org go list -m
...
- [ ] 4.3 Add changelog extraction (`git-cliff --current --strip header -o
  RELEASE_NOTES.md`) and GitHub Release creation (`softprops/action-gh-release`) with
  `body_path: RELEASE_NOTES.md`.
- [ ] 4.4 Add the develop merge-back step, mirroring the Rust SDK's `release.yaml`
  (checkout `develop`, `git merge origin/master`, push).

## 5. Documentation

- [ ] 5.1 Add `RELEASE.md` documenting the release process: how to trigger
...
- [ ] 5.2 Update `README.md`: add CI and release status badges (mirroring the Rust SDK's
  README badge row), link to `RELEASE.md`, add a `go.dev` reference badge for the module.

## 6. Verification

- [ ] 6.1 Confirm `golangci-lint run ./...` and `go test -race ./...` pass locally before
