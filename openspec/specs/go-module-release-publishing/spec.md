# go-module-release-publishing Specification

## Purpose
Define automated GitHub Release creation and Go module proxy indexing triggered by
version tags on the Go SDK module, so every version bump on `master` produces an
installable, released module — adapted from the Rust SDK's crate-release-publishing
process to Go's tag-based, registry-free publishing model.
...
so `develop` includes the version bump and changelog update produced by the release. This
push SHALL be attributed to an actor configured to bypass the `develop` branch ruleset's
required-check rules, since the merge commit it creates has no prior CodeQL scan result
and cannot satisfy a required status-check rule that expects one before it can land.
...
#### Scenario: Merge-back push is rejected by branch protection
- **WHEN** the merge-back push is attributed to an actor not covered by the `develop`
  ruleset's bypass list (e.g. the default `GITHUB_TOKEN`'s actions-bot identity)
- **THEN** the push is rejected (`GH013`) because no required check result can exist yet
  for a commit that has never been pushed before
