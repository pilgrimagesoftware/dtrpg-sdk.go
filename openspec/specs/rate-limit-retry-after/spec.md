# rate-limit-retry-after Specification

## Purpose
Define passive parsing and exposure of the `Retry-After` header on library API errors, so
callers can back off without the SDK implementing retry logic itself.
